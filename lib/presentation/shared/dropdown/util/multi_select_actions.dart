...
class MultiSelectActions<T> {
  List<T> onItemCheckedChange(
      List<T> selectedValues, T itemValue, bool checked,int maxSelectedItems) {
    if (checked) {
    ...
    if(selectedValues.length > maxSelectedItems)
    {
      var itemsToRemove = selectedValues.length - maxSelectedItems;
      selectedValues.removeRange(0, itemsToRemove);
    }
