...
class MultiSelectBottomSheetWithOptions<T> extends StatefulWidget
  ...
  MultiSelectBottomSheetWithOptions({
    required this.items,
    ...
    if(this.maxSelectedItems==-1){
      this.maxSelectedItems=this.items.length;
    }
  ...
  @override
  _MultiSelectBottomSheetWithOptionsState<T> createState() =>
      _MultiSelectBottomSheetWithOptionsState<T>(items);
}

class _MultiSelectBottomSheetWithOptionsState<T> extends State<MultiSelectBottomSheetWithOptions<T>> {
  List<T> _selectedValues = [];
  bool _showSearch = false;
  List<MultiSelectItem<T>> _items;
  int maxItemsSelected=0;

  _MultiSelectBottomSheetWithOptionsState(this._items);

  @override
  void initState() {
    super.initState();
    _selectedValues.addAll(widget.initialValue);
    maxItemsSelected=widget.maxSelectedItems;

    if(_selectedValues.length > maxItemsSelected) {

     var  itemsToBeRemoved = _selectedValues.length - maxItemsSelected;
     _selectedValues.removeRange(0,itemsToBeRemoved);
        ...
        onChanged: (checked) {
          setState(() {
            _selectedValues = widget.onItemCheckedChange(
                _selectedValues, item.value, checked!,widget.maxSelectedItems);
            ...
            for (int i = 0; i < _items.length; i++) {
              if (_selectedValues.contains(_items[i].value) == false) {
